...
/** Orrery planets: cross-hatch shading plus parchment feature maps. */
export const davinciPlanetVertexShader = `
  varying vec3 vNormal;
  varying vec3 vViewNormal;
  varying vec3 vLocalPosition;
  varying vec2 vUv;
...
export const davinciPlanetFragmentShader = `
  uniform vec3 uLightDirection;
  uniform vec3 uInkColor;
  uniform vec3 uPaperColor;
  uniform float uHatchDensity;
  uniform float uLineWeight;
  uniform sampler2D uFeatureMap;
  ...
  void main() {
    ...
    vec3 featureSample = texture2D(uFeatureMap, vUv).rgb;
    float featureInk = 1.0 - dot(featureSample, vec3(0.299, 0.587, 0.114));
    inkIntensity = clamp(inkIntensity + featureInk * uFeatureStrength, 0.0, 1.0);
